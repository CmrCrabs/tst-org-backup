...
To install the extension, open =about:debugging= and load =tst-org-sync.zip= from this repo under Temporary Extensions.
...
(httpd-servlet tst-org-sync/read-file text/plain ()
  (insert-file-contents my/local-file-path))

(httpd-servlet tst-org-sync/read-timestamp text/plain ()
...
(defun httpd/tst-org-sync/write (proc path &rest args)
  "PROC PATH ARGS."
...
- [[http://localhost/8080/tst-org-sync/read-file]] (should return the raw org file as a response)
- [[http://localhost/8080/tst-org-sync/read-timestamp]] (should return the modification time of the file in the same format as defined in =C-h f current-time=)
- [[http://localhost/8080/tst-org-sync/write]] (up to you)
